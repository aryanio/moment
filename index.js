...
import * as jalaali from './jalaali';

/************************************
 Constants
 ************************************/

const formattingTokens = /(\[[^\[]*\])|(\\)?j(Mo|MM?M?M?|Do|DDDo|DD?D?D?|w[o|w]?|YYYYY|YYYY|YY|gg(ggg?)?|)|(\\)?(Mo|MM?M?M?|Do|DDDo|DD?D?D?|ddd?d?|do?|w[o|w]?|W[o|W]?|YYYYY|YYYY|YY|gg(ggg?)?|GG(GGG?)?|e|E|a|A|hh?|HH?|mm?|ss?|SS?S?|X|zz?|ZZ?|.)/g
...
function normalizeUnits(units) {
    if (units) {
        const lowered = units.toLowerCase();
...
jMoment.fn.jDaysInMonth = function () {
    const year = this.jYear();
    const month = this.jMonth();
    if (month >= 0 && month < 6) return 31;
    else if (month >= 6 && month < 11) return 30;
    else return jMoment.jIsLeapYear(year) ? 30 : 29;
};
